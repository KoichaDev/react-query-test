...
export const useAddSuperHeroData = () => {
	...
	return useMutation(addSuperHero, {
		// onMutate is called before the mutation is fired and is passed on the same variables the mutation functions will recieve
		onMutate: async (newHero) => {
			// 1. We want to cancel any outgoing refetches, so they don't overwrite out optimistic update
			await queryClient.cancelQueries('super-heroes');

			// 2. Need to get hold of the current data before we make any update. This will help us to rollback in case the mutation fails
			const previousHeroData = queryClient.getQueryData('super-heroes');

			// 3. Update the new "mutation" data
			queryClient.setQueriesData('super-heroes', (oldQueryData) => {
				return {
					...
					data: [
						...
						{
							// The id is generated by incrementing + 1 to the json server
			...
			return {
				previousHeroData, // This is used to rollback data in case mutation errors out
			};
		},
		// This mutation encounters if there is an error
		onError: (_error, _hero, context) => {
			// context gives ut access from the previous data onMutated callback and set it as the query data when there is an error
			queryClient.setQueryData('super-heroes', context.previousHeroData);
		},
		// This mutation is called either if it's successful, or when encounters an error
		onSettled: () => {
			// This will ensure the client state is in-sync with the json server state. The user will of course not notice the difference
